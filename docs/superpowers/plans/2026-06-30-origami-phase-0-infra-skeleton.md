...
- 컨테이너는 호스트 포트를 외부에 열지 않는다. `127.0.0.1:4500:4500` 만 로컬 확인용으로 노출. 외부 노출은 `edge_shared` 네트워크 + Caddy.
...
- 컨테이너 이름은 `origami-app`, compose 프로젝트 이름은 `origami`, 노출 포트는 `4500`.
...
Then: `curl -s http://localhost:4500/api/health`
...
Then:
- `curl -s http://localhost:4500/api/health` → `{"ok":true,"version":"dev"}`
- `curl -s http://localhost:4500/` 응답에 `hello, origami` 포함
- `curl -s http://localhost:4500/non/existent/route` 응답에 `hello, origami` 포함 (SPA fallback)
...
**Interfaces:**
- Produces: 컨테이너 이미지를 만들어 `4500` 포트에 SPA + `/api/health` 를 서빙한다. compose 파일은 외부 `edge_shared` 네트워크에 합류하고 `127.0.0.1:4500:4500` 만 로컬 노출.
...
Run: `curl -s http://localhost:4500/` 응답에 `hello, origami` 포함 확인.
...
git commit -m "Phase 0/T3: Dockerfile + docker-compose (edge_shared, 포트 4500)"
```
...
DEADLINE=$(( $(date +%s) + 60 ))
while true; do
  if curl -fsS http://127.0.0.1:4500/api/health >/dev/null; then
    echo "[deploy] healthy"
    break
  fi
...
```bash
npm install
npm run dev:server   # 백엔드 :4500
npm run dev:client   # 프론트 :5173 (api는 4500으로 프록시)
```
...
```bash
docker ps --format '{{.Names}}\t{{.Ports}}' | grep ':4500' || echo "포트 4500 OK"
ss -tln | grep -E ':4500\b' || echo "포트 4500 OK"
```
...
http://origami.myazit.kr {
    import common
    reverse_proxy origami-app:4500
}
```
...
- **헬스체크 실패**: `docker exec origami-app wget -qO- http://localhost:4500/api/health`. 컨테이너 내부에서 실패하면 빌드 산출물(`dist/server/index.js`)이 들어갔는지 확인.
...
- [ ] `docs/superpowers/specs/2026-06-30-origami-cp-generator-design.md` §8 TBD 중 "포트 4500 충돌" 항목 해소 (확정된 포트 번호 기록)
