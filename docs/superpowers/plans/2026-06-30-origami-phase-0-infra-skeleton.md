...
- 컨테이너는 호스트 포트를 외부에 열지 않는다. `127.0.0.1:4600:4600` 만 로컬 확인용으로 노출. 외부 노출은 `edge_shared` 네트워크 + Caddy.
...
- 컨테이너 이름은 `origami-app`, compose 프로젝트 이름은 `origami`, 노출 포트는 `4600`.
...
Then: `curl -s http://localhost:4600/api/health`
...
Then:
- `curl -s http://localhost:4600/api/health` → `{"ok":true,"version":"dev"}`
- `curl -s http://localhost:4600/` 응답에 `hello, origami` 포함
- `curl -s http://localhost:4600/non/existent/route` 응답에 `hello, origami` 포함 (SPA fallback)
...
**Interfaces:**
- Produces: 컨테이너 이미지를 만들어 `4600` 포트에 SPA + `/api/health` 를 서빙한다. compose 파일은 외부 `edge_shared` 네트워크에 합류하고 `127.0.0.1:4600:4600` 만 로컬 노출.
...
services:
  app:
    ...
    networks:
      - edge_shared
    healthcheck:
      test: ["CMD", "wget", "-q", "--spider", "http://localhost:4600/api/health"]
      interval: 30s
      timeout: 5s
      retries: 3
      start_period: 10s
...
Run: `curl -s http://localhost:4600/` 응답에 `hello, origami` 포함 확인.
...
git commit -m "Phase 0/T3: Dockerfile + docker-compose (edge_shared, 포트 4600)"
```
...
DEADLINE=$(( $(date +%s) + 60 ))
while true; do
  if curl -fsS http://127.0.0.1:4600/api/health >/dev/null; then
    echo "[deploy] healthy"
    break
  fi
...
- [ ] `docs/superpowers/specs/2026-06-30-origami-cp-generator-design.md` §8 TBD 중 "포트 4600 충돌" 항목 해소 (확정된 포트 번호 기록)
